...
class Treatment:
    ...
    message = {
        'faq':
            " \nБот збирає інформацію про самопочуття людини " +
            "протягом дня, цифра від 1 до 5, де 1 погане самопочуття" +
            " 5 все добре. Якщо протягом одного дня користувач " +
            "вводить цифри повторно то залишається остання. Якщо " +
            "дані не валідні бот вказує на це користувачу.",
        'stop': "Goodby!\nSee you later!",
        'invalid':
    ...
    def week(self, message):
        if self.if_user_exists(message):
            ...
            return f'Dear {message.from_user.first_name},'\
                f' you are already not registered!'\
                f"{self.message['invalid']}"

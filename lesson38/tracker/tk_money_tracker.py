...
class ShowUsers(tk.simpledialog.Dialog):
    def __init__(self,
                 parent,
                 title,
                 records):
    ...
    def body(self, frame):
        def table():
            ...
            for row in self.records:
                tree.insert("", tk.END, values=(row['id'],
                                                row['username'],
                                                ))
...
class MyDialog(tk.simpledialog.Dialog):
    ...
    def body(self, frame):
        ...
        self.user_id_label = tk.Label(
            frame, width=25, text="user_id")
        self.user_id_label.pack()
        self.user_id_box = tk.Entry(frame, width=25)
        self.user_id_box.insert(0, str(self.user_id or ''))
    ...
    @staticmethod
    def user_id_validator(value_str):
    ...
    def ok_pressed(self):
        ...
        self.user_id = self.user_id_validator(
            self.user_id_box.get())
        ...
        if self.money_amount is None:
            ...
            mb.showerror("Mistake",
                         "Введіть дані в форматі ID (приклад 1)")
        if all((self.money_amount, self.date, self.user_id)):
            self.destroy()
...
class TkInterface:
    ...
    def main_menu(self):
        ...
        smart_menu.add_command(label="view all users",
    ...
    def delete(self):
        """Метод відповідає за видалення запису"""
        id_row = simpledialog.askinteger("Enter ID",
                                         "Введіть ID запису для видалення")
        records = self.wallet.get_all_records()
        for record in records:
            if record['id'] == id_row:
                self.wallet.delete(record['id'])
                break
        else:
            mb.askyesno(
                title='Mistake',
                message='Такого запису не інснує')
    ...
    def add(self):
        ...
        (money_amount, transaction_date,
         reason, contragent, user_id) = self.tk_add_dialog(self.window)
        if money_amount:
            self.wallet.new_record(
                user_id,
                money_amount,
                transaction_date,
                reason,
    ...
    def tk_show_users_dialog(self, window, records):
        ShowUsers(title="ShowUsers", parent=window, records=records)

    def to_public_users(self, window, records):
        if not records:
            self.show_info("There is no information" +
                           "\non your request")
        else:
            self.tk_show_users_dialog(
                window,
                records)
    ...
    def view_all_users(self):
        records = self.wallet.get_all_users()
        self.to_public_users(self.window, records)

    def get_main_source_of_income(self, window):
        records = self.wallet.get_main_source_of_income()
        self.to_public(window, records)

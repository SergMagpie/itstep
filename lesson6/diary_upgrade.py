...
# one could write a construction "while day != 'exit'"
# declare a variable "day" in advance
while True:
    day = input('''Enter day of week: Monday, Tuesday, Wednesday, Thursday,
Friday, Saturday, Sunday or number of day or exit for exit: ''')
# start of a series of checks
    if day.isdigit():
        number = (int(day) - 1)
        if 0 <= number < 7:
    ...
    if day in day_of_week:
        ...
        print(*diary[day], sep='\n')
        print()
# if the output is to be accepted strictly in lower case,
# move it to the beginning of the test series
    elif day == 'Exit':
        break
    else:
        print('You make mistake')
# for beauty I decided to separate the request with an empty line
        print()
